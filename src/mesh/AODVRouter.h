...
// AODV Packet Type Markers (stored in decoded.portnum for identification)
// We repurpose unused PortNum values for AODV control packets
#define AODV_PORTNUM_RREQ ((meshtastic_PortNum)250)  // Custom port for RREQ packets
#define AODV_PORTNUM_RREP ((meshtastic_PortNum)251)  // Custom port for RREP packets  
#define AODV_PORTNUM_RERR ((meshtastic_PortNum)252)  // Custom port for RERR packets
...
struct AODVRouteEntry {
    ...
    uint8_t destSeqNum;          // Destination sequence number
...
class AODVRouter : public FloodingRouter
{
  ...
  private:
    ...
    // Send RREP - header-based (physical layer)
    void sendRREP(NodeNum originatorAddr, NodeNum destinationAddr, uint32_t rreqId,
