...
std::string MeshPacketSerializer::JsonSerialize(const meshtastic_MeshPacket *mp, bool shouldLog)
{
    ...
    if (mp->which_payload_variant == meshtastic_MeshPacket_decoded_tag) {
        ...
        case meshtastic_PortNum_ROUTING_APP: {
            ...
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &meshtastic_Routing_msg, &scratch)) {
                jsonObj["payload"]["request_id"] = (unsigned int)mp->decoded.request_id;
                ...
                case meshtastic_Routing_error_reason_tag:
                    jsonObj["payload"]["error_reason"] = (int)scratch.error_reason;
                ...
                msgType = "ROUTING";
                if (shouldLog) {
                    LOG_ERROR("Error decoding proto for Routing message!");
                    return "";
                }
            }
            break;
        ...
        case meshtastic_PortNum_AODV_ROUTING_APP: {
            ...
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &meshtastic_AODV_msg, &scratch)) {
                ...
                case meshtastic_AODV_rreq_tag:
                    msgType = "AODV_RREQ";
                    jsonObj["payload"]["destination"] = (unsigned int)scratch.variant.rreq.destination;
                    break;
                ...
                msgType = "AODV";
                if (shouldLog)
                    LOG_ERROR("Error decoding proto for AODV message!");
            }
            break;
        }
        case meshtastic_PortNum_SDN_APP: {
            ...
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &meshtastic_SDN_msg, &scratch)) {
                ...
                msgType = "SDN";
                if (shouldLog)
                    LOG_ERROR("Error decoding proto for SDN message!");
            }
            break;
        }
    ...
    jsonObj["relay_node"] = (unsigned int)mp->relay_node;
    jsonObj["next_hop"] = (unsigned int)mp->next_hop;
